...
C      DIMENSION A(MAXISL,12),B(MAXISL,12),
C     & C(MAXISL,12,1922:2100),NODE(MAXNODE)
...
C..
     ...
     .            'JUN','JUL','AUG','SEP'/
C      DATA DAYS_MONTH/31,30,31,31,28,31,30,31,30,31,31,30/
C      DATA DAYS_MONTH_LEAP/31,30,31,31,29,31,30,31,30,31,31,30/
...
C***************************MONTHS TO DAYS************************************     
      INTEGER NUMDIV(MAXISL),NUMDRN(MAXISL),LISTDIV(MAXISL,65),
     &     LISTDRN(MAXISL,65)
...
C*
C      WRITE(*,1800)
C 1800 FORMAT(/'ENTER TYPE OF DRAINAGE CONCENTRATION DATA 
C     &      (ENTER 1 FOR TDS,2 FOR CL): ',$)
...
C         WRITE(*,928)
C 928     FORMAT(/' THERE IS AN ASCII FILE CREATED NAMED junk.txt.'/
C     &        ' TO CONVERT THIS TO A DSS FILE, JUST TYPE:'//
C     &        '       dssts <junk.txt'//,
C     &        ' AFTER THE MODEL RUN IS COMPLETED.'/
C     &        ' AFTER MAKING SURE THAT THE DSS FILE IS PROPERLY'/
C     &        ' CREATED, YOU CAN DELETE THE FILE junk.txt'//
C     &        ' ENTER THE NAME OF THE DSS FILE: ',$)
...
C-----1. READS INPUT FILES FROM THE NEW PICKWY PROGRAM (YEAR IS PART OF FILENAME)
	...
	CALL GETENV('DICU5.30',DICU5_30)
	CALL GETENV('GW_RATES.TXT',GROUNDWATER)
...
C++++++++++++++++GROUNDWATER RATIOES++++++++++++++++++++++++++++++++
      ...
      open(72,file='GWAmount2isl.txt',status='unknown')
            !add 12/17/2015 to calculate the island groundwater
...
C      ENDDO
C      DO Lt = 1, 114
C         READ(51,*)tempisl, temp1,temp2,temp3,temp4,temp5,temp6
C         GWINP(tempisl,1) = temp1   !May
C         GWINP(tempisl,2) = temp2   !Jun
C         GWINP(tempisl,3) = temp3   !Jul
C         GWINP(tempisl,4) = temp4   !Aug
C         GWINP(tempisl,5) = temp5   !Sep
C         GWINP(tempisl,6) = temp6   !Oct         
C      ENDDO
...
C..
 ...
 208        CONTINUE
         ...
         DO 300 I=1,MAXISL
            DO 200 M=1,DAYS_YR
C               WX(I,M)=PREC(I,M,YR) - TCU(I,M,YR)
                WX(I,M)=PREC(I,M,YR)
C               RO(I,M)=PREC(I,M,YR) - TCU(I,M,YR)
                RO(I,M)=PREC(I,M,YR)
               IF(RO(I,M).LT.0) THEN
                  RO(I,M)=0.
               ENDIF
...
C--------WRITE ISLAND DATA
         ...
         WRITE(20,11)(I,(AW(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
         
         WRITE(20,'(40HMONTHLY SEEPAGE FOR EACH ISLAND, ACRE-FT)')      
         WRITE(20,11)(I,(S(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
         
         WRITE(20,'(45HMONTHLY PRECIPIATION FOR EACH ISLAND, ACRE-FT)')
         WRITE(20,11)(I,(PREC(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
         WRITE(25,'(45HMONTHLY PRECIPIATION FOR EACH ISLAND, ACRE-FT)')
         WRITE(25,11)(I,(PREC(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
         
         WRITE(20,*)'(47HMONTH TOTAL CONSUMPTIVE USE EACH ISL,ACRE-FT)'
         WRITE(20,11)(I,(TCU(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
         WRITE(25,*)'(47HMONTH TOTAL CONSUMPTIVE USE EACH ISL,ACRE-FT)'
         WRITE(25,11)(I,(TCU(I,M,YR),M=1,DAYS_YR),I=1,MAXISL)
...
C--------WRITE(20,('(I4,4X,12F6.3)')) (I,(C(I,M,YR),M=1,12),I=1,MAXISL)
         ...
         WRITE(20,*)'(48HISLAND DRAINAGE ALLOCATION FACTORS FOR NODES)'
         DO I=1,MAXISL
            DO N=1,NUMDRN(I)
               NN=LISTDRN(I,N)
         ...
         IF (CONC.EQ.1) THEN
           WRITE(20,*)'(49HMONTHLY ISLAND DRAINAGE TDS CONCENT IN MG/L)'
         ENDIF
         IF (CONC.EQ.2) THEN
           WRITE(20,*)'(49HMONTHLY ISLAND DRAINAGE  CL CONCENT IN MG/L)'
         ENDIF
         WRITE(20,11) (I,(DS(I,M),M=1,DAYS_YR),I=1,MAXISL)
         ...
         WRITE(20,11)(I,(RO(I,M),M=1,DAYS_YR),I=1,MAXISL)
C..---------
         WRITE(20,*)'(38HISLAND NET ATM WATER EXCHANGE, ACRE-FT)'
         WRITE(20,11)(I,(WX(I,M),M=1,DAYS_YR),I=1,MAXISL)
C..---------
         WRITE(25,*)'(38HNET ISLAND ATM WATER EXCHANGE, ACRE-FT)'
         WRITE(25,11)(I,(WX(I,M),M=1,DAYS_YR),I=1,MAXISL)
C..------
...
C..
         ...
         DO 500 I=1,MAXISL
            DO 531 IG = 1, DAYS_YR
C assumed 40% applied water comes from groundwater pumping.  6/24/2014
C assumed 40% runoff percolates into ground.  6/24/2014
...
C                if (docregion(i).eq."H") gwrate=0.6
             else
                gwrate = GW_RATE(YR)  
             endif
               ...
               IF(IG.GT.DAYS_MONTH_LEAP(11).AND.
     &                IG.LE.DAYS_MONTH_LEAP(12)) THEN
CC                    tempt = AW(I,IG,YR)/IIE(I)+S(I,IG,YR)
CC                    tempt1 = AW(I,IG,YR)/IIE(I)/tempt
CC                    tempt2 = (S(I,IG,YR))/tempt 
CC                    GWAMOUNT1(I,IG) = GWINP(I,5)*tempt1
CC                    GWAMOUNT2(I,IG) = GWINP(I,5)*tempt2
C                     GWAMOUNT1(I,IG) = GWINP(I,5)  !adjust for sep
C                     GWAMOUNT2(I,IG) = 0.
               ENDIF       
               IF(IG.LE.DAYS_MONTH_LEAP(1)) THEN
C                    GWAMOUNT1(I,IG) = GWINP(I,6)  !adjust for oct
C                    GWAMOUNT2(I,IG) = 0.
               ENDIF   
...
C            DO 535 IG = 1, DAYS_YR
C               IF(MOD(YR,4).EQ.0) LPYR=1
C               tempt = AW(I,IG,YR)/IIE(I)+S(I,IG,YR)+tempdrns
C               tempt1 = AW(I,IG,YR)/IIE(I)/tempt
C               tempt2 = (S(I,IG,YR)+tempdrns)/tempt
C               IF (LPYR.EQ.1) THEN                  
C                   IF(IG.GT.213) THEN
C                      DO MM = 12,12
C                         IF(IG.GT.DAYS_MONTH_LEAP(MM-1).AND.
C     &                      IG.LE.DAYS_MONTH_LEAP(MM)) THEN            
C                            GWAMOUNT1(I,IG) = GWINP(I,MM-7)*tempt1
C                            GWAMOUNT2(I,IG) = GWINP(I,MM-7)*tempt2
C                          ENDIF
C                       ENDDO
C                   ELSE 
C                      IF(IG.LE.DAYS_MONTH_LEAP(1)) THEN
C                         GWAMOUNT1(I,IG) = GWINP(I,6)*tempt1
C                         GWAMOUNT2(I,IG) = GWINP(I,6)*tempt2
C                      ENDIF
C                   ENDIF  
C               ELSE
C                  IF (IG.GT.212) THEN
C                     DO MM = 12,12
C                        IF(IG.GT.DAYS_MONTH(MM-1).AND.
C     &                     IG.LE.DAYS_MONTH(MM)) THEN
C                           GWAMOUNT1(I,IG) = GWINP(I,MM-7)*tempt1
C                           GWAMOUNT2(I,IG) = GWINP(I,MM-7)*tempt2 
C                        ENDIF
C                     ENDDO       
C                  ELSE
C                     IF(IG.LE.DAYS_MONTH(1)) THEN
C                        GWAMOUNT1(I,IG) = GWINP(I,6)*tempt1
C                        GWAMOUNT2(I,IG) = GWINP(I,6)*tempt2             
C                     ENDIF                          
C                  ENDIF                      
...
C..
         ...
         WRITE(21,1000)(I,(ID(I,M),M=1,DAYS_YR),TAID(I),I=1,MAXISL)
         WRITE(21,1005)(TMDID(M),M=1,DAYS_YR), TADID
         WRITE(21,1002)YR,(WMONTH(M),M=1,12)
         WRITE(21,1000)(I,(D(I,M),M=1,DAYS_YR),TAD(I),I=1,MAXISL)
         WRITE(21,1006)(TMDD(M),M=1,DAYS_YR), TADD
         WRITE(21,1012)(CDNET(M),M=1,DAYS_YR), CDDNET
...
CNM-10/04/95
         ...
         WRITE(21,1024)(I,(ID1(I,M),M=1,DAYS_YR),I=1,MAXISL)
...
C..
         ...
         WRITE(22,1019)(N,(NID(N,M),M=1,DAYS_YR),N=1,MAXNODE)
         WRITE(22,1020)(NTMDID(M),M=1,DAYS_YR)
         WRITE(22,1014)YR,(WMONTH(M),M=1,12)
         WRITE(22,1019)(N,(ND(N,M),M=1,DAYS_YR),N=1,MAXNODE)
         WRITE(22,1021)(NTMDD(M),M=1,DAYS_YR)
         WRITE(22,1015)(NCDNET(M),M=1,DAYS_YR)
C..
...
C..
         ...
         WRITE(23,1009)(N,(NID(N,M),M=1,DAYS_YR),N=1,MAXNODE)
         WRITE(23,1010)(NTMDID(M),M=1,DAYS_YR)
         WRITE(23,1004)YR,(WMONTH(M),M=1,12)
         WRITE(23,1009)(N,(ND(N,M),M=1,DAYS_YR),N=1,MAXNODE)
         WRITE(23,1011)(NTMDD(M),M=1,DAYS_YR)
         WRITE(23,1016)(NCDNET(M),M=1,DAYS_YR)
...
C...
 ...
 953        CONTINUE
            IF(IFLAG.NE.0) WRITE(24,1049) N,(NDS(N,M),M=1,DAYS_YR)
 955     CONTINUE
C...
C...
...
CNM2--------------
 ...
 770           CONTINUE
               IF(JIDFLAG.EQ.1) WRITE(26,23) I,N,(DIV(I,M),M=1,DAYS_YR)
               IF(JDFLAG.EQ.1) WRITE(27,23) I,N,(DRN(I,M),M=1,DAYS_YR)
 776        CONTINUE
 780     CONTINUE
...
C            WRITE(QUANTITY,'(A3,1H.,I4.4)')WMONTH(M),YR
C            OPEN(IUNIT,FILE=QUANTITY,FORM='FORMATTED',STATUS='UNKNOWN')      
C            WRITE(IUNIT,1007) WMONTH(M),YR
...
C               WRITE(IUNIT,1008) N,ND(N,M),NID(N,M),NID1(N,M),NID2(N,M)   
...
c---------------------add groundwater amount 2
 ...
 2929      FORMAT('A=DICU-ISLAND  B=',I4,
     ...
     &       '01OCT1921 2400')
      ...
      WRITE(72,2913)
      CLOSE(72)
c---------------------end of groundwater amount 2 
      IF(ANSWER.EQ.'Y' .OR. ANSWER.EQ.'y')THEN
         DO IFLOW=1,3
            IF(IFLOW.EQ.1) THEN
               OPEN(FILE='junk1_1.txt',UNIT=52,STATUS='UNKNOWN')
               OPEN(FILE='junk1_2.txt',UNIT=53,STATUS='UNKNOWN')
            ELSE IF(IFLOW.EQ.2) THEN
               OPEN(FILE='junk2_1.txt',UNIT=52,STATUS='UNKNOWN')
               OPEN(FILE='junk2_2.txt',UNIT=53,STATUS='UNKNOWN')
            ELSE
               OPEN(FILE='junk3_1.txt',UNIT=52,STATUS='UNKNOWN')
               OPEN(FILE='junk3_2.txt',UNIT=53,STATUS='UNKNOWN')
            ENDIF
            WRITE(52,930)FLE
            WRITE(53,930)FLE
 930        FORMAT(A120)
            ...
            DO K=1,100               
               ...
               IF(NDFLG(N))THEN
                  IF(N.NE.193)THEN
                     IF(IFLOW.EQ.1)THEN
                        WRITE(52,1910)N
 1910                   FORMAT('A=DICU-HIST+NODE  B=',I4,
     &               '  C=DRAIN-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
                     ELSEIF(IFLOW.EQ.2)THEN
                        WRITE(52,1911)N
 1911                   FORMAT('A=DICU-HIST+NODE  B=',I4,
     &                  '  C=DIV-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/ 
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
                     ELSEIF(IFLOW.EQ.3)THEN
                        WRITE(52,1912)N
 1912                   FORMAT('A=DICU-HIST+NODE  B=',I4,
     &                 '  C=SEEP-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
                     ENDIF
...
C--------------------Name for node 193 is changed to BBID (Byron Bethany)
                     IF(IFLOW.EQ.1)THEN
                        WRITE(52,1980)
 1980                   FORMAT('A=DICU-HIST+RSVR  B=','BBID',
     &               '  C=DRAIN-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
                     ELSEIF(IFLOW.EQ.2)THEN
                        WRITE(52,1981)
 1981                   FORMAT('A=DICU-HIST+RSVR  B=','BBID',
     &                '  C=DIV-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
                     ELSEIF(IFLOW.EQ.3)THEN
                        WRITE(52,1982)
 1982                   FORMAT('A=DICU-HIST+RSVR  B=','BBID',
     &                '  C=SEEP-FLOW  D=01JAN1920  E=1DAY  F=DWR-BDO'/
     &                       'CFS'/
     &                       'PER-AVER'/
     &                       '01OCT1921 2400')
...
C--------------------Name for node 193 is changed to BBID (Byron Bethany)
                  ...
                  DO YR=iyr1,iyr2
                     ...
                     DO M=1,DAYS_YR         
                        IF(IFLOW.EQ.1)THEN
                           WRITE(53,1920)QDRN(N,YR,M) 
                        ELSEIF(IFLOW.EQ.2)THEN
                           WRITE(53,1921)QIRR(N,YR,M)
                        ELSEIF(IFLOW.EQ.3)THEN
                           WRITE(53,1922)QSEEP(N,YR,M) 
                        ENDIF
                     ENDDO
                  ENDDO
                  WRITE(53,1940)
